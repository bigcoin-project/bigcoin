...
*Setup instructions for a Gitian build of Bigcoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Bigcoin
...
- [Building Bigcoin Core](#building-bigcoin-core)
...
Clone the git repositories for bigcoin and Gitian.
...
in the bigcoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Bigcoin Core
----------------

To build Bigcoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the bigcoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/bigcoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/bigcoin-project/bigcoin
...
./bin/gbuild --commit bigcoin=${COMMIT} --url bigcoin=${URL} ../bigcoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit bigcoin=${COMMIT} --url bigcoin=${URL} ../bigcoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit bigcoin=${COMMIT} --url bigcoin=${URL} ../bigcoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the bigcoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../bigcoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/bigcoin
SIGPATH=/some/root/path/bigcoin-detached-sigs

./bin/gbuild --url bigcoin=${BTCPATH},signature=${SIGPATH} ../bigcoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/bigcoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/bigcoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/bigcoin-osx-build.assert
```
...
[bigcoind/gitian.sigs.bgc](https://github.com/bigcoin-project/gitian.sigs.bgc/) repository, or if that's not possible create a pull
